...
def evaluate_program(program: dspy.Module, eval_rows: list[dict[str, Any]], judge: JudgeDSPY) -> dict[str, Any]:
    ...
    for idx, row in enumerate(eval_rows, start=1):
        ...
        # Use the hanb1n syntax judge evaluation
        judge_result = judge._evaluate_query_dsl_syntax(generated_query_dsl=predicted)
    ...
    summary = {
        ...
        "avg_judge_score": sum(item["judge_result"].get("score", 1.0 if item["judge_result"].get("is_valid") else 0.0) for item in results) / total if total else 0.0,
        "results": results,
    }
    return summary
